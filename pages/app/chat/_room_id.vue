        ...
        <div v-for="(msg, index) in posts" :key="index">
          <div v-if="!msg.text.match(regexp)">
            <v-message-date
              ...
              :message="msg"
            ></v-message-date>
            <v-layout column :align-end="msg.send_uid === $auth.state.user.id">
              <v-flex>
                <v-messageTimestamp
            ...
            <v-layout column :align-end="msg.send_uid === $auth.state.user.id">
              <v-flex>
                <spot-card
                  class="callout right"
                  :class="
                    msg.send_uid === $auth.state.user.id ? 'left' : 'right'
                  "
...
export default {
  ...
  created() {
    ...
    this.setPostsRef(
      db
        .collection(`/chat_rooms/${this.$route.params.room_id}/messages`)
        .orderBy('created')
    ).then(() => {
      this.refreshRender()
    })
