        ...
        <v-stepper v-model="e1">
          ...
          <v-stepper-items>
                      ...
                      <template v-slot:append>
                        <v-fade-transition leave-absolute>
                          <v-progress-circular
                            ...
                            indeterminate
                          ></v-progress-circular>
                          <v-icon v-else @click="getLocation"
            ...
            <v-stepper-content :key="`5-content`" :step="5">
              <!-- 登録画面 -->
              <v-checkbox v-model="agreementService" :rules="[rules.required]">
              ...
              <v-checkbox v-model="agreementPolicy" :rules="[rules.required]">
                <template v-slot:label>
                  <a
                    href="#"
                    @click.stop.prevent="
              ...
              <v-dialog v-model="dialog" absolute max-width="400" persistent>
                <v-card>
                  ...
                  <v-card-actions>
                    <v-btn
                      text
                      @click=";(agreementPolicy = false), (dialog = false)"
                    >
                      No
                    </v-btn>
                    <div class="flex-grow-1"></div>
                    <v-btn
                      class="white--text"
                      color="deep-purple accent-4"
                      @click=";(agreementPolicy = true), (dialog = false)"
                    >
                      Yes
                    </v-btn>
                  </v-card-actions>
  ...
  methods: {
    ...
    imageUploader() {
      this.croppaImg.generateBlob((blob) => {
        ...
        this.$axios.$post(`${process.env.apiBaseUrl}imageupload`, this.formData)
      })
    },
    currentValue() {
    ...
    register() {
      ...
      this.croppaImg.generateBlob((blob) => {
        ...
        this.$axios
          .$post(`${process.env.apiBaseUrl}auth/register`, this.formData)
          .then((res) => {
