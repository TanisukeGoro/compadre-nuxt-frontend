    ...
    <v-col class="ma-3">
      ...
      >
        ...
        <v-card-text class="headline font-weight-bold">
          <div v-for="(text, index) in cardText" :key="index">
            {{ text }}
          </div>
        </v-card-text>
        <v-btn-toggle multiple mandatory>
          <v-btn text width="100px" height="30px" @click="previousCard"
            >previous</v-btn
          >
          <v-btn color="accent" height="30px" width="100px" @click="nextCard"
            >next</v-btn
          >
        </v-btn-toggle>
      </v-card>
